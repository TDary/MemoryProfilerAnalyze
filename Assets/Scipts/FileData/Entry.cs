...
namespace Scipts.FileData
{
    ...
    struct EntryHeader
    {
        [FieldOffset(0)] public readonly EntryFormat Format;

        [FieldOffset(2)] public readonly uint BlockIndex;

        //the data in this meta value needs to be interpreted based on entry format, can be entry size or entries count
        [FieldOffset(6)] public readonly uint EntriesMeta;

        //the Data in this meta value needs to be interpreted based on EntryFormat
        [FieldOffset(10)] public readonly ulong HeaderMeta;
    }
    ...
    unsafe struct Entry : IDisposable
    {
        ...
        public Entry(EntryHeader header)
        {
            ...
            switch (Header.Format)
            {
                ...
                case EntryFormat.DynamicSizeElementArray:
                    ...
                    m_AdditionalEntryStorage = (long*)UnsafeUtility.Malloc(sizeof(long) * header.EntriesMeta,
        ...
        public unsafe long* GetAdditionalStoragePtr()
        {
            return m_AdditionalEntryStorage;
        }
    }
}