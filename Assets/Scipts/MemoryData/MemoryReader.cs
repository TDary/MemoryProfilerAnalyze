...
namespace MemoryAnalyze
{
    ...
    abstract public class AbstractMemoryReader
    {
        ...
        {
            // strings differ from any other data type in the CLR (other than arrays) in that their size isn抰 fixed.
            // Normally the .NET GC knows the size of an object when it抯 being allocated, because it抯 based on the
            // size of the fields/properties within the object and they don抰 change. However in .NET a string object
            // doesn抰 contain a pointer to the actual string data, which is then stored elsewhere on the heap.
