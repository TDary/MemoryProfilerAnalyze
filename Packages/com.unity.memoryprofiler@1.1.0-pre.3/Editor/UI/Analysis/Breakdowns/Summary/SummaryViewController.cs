    ...
    class SummaryViewController : ViewController, IViewControllerWithVisibilityEvents
    {
        ...
        public void ExportSummaryData()
        {
            var path = EditorUtility.SaveFilePanel(
                ...
                $"MPT_MemorySummary_{DateTime.Now.ToString("yyyyMMdd_HHmm")}", 
                "tab"
            );
            
            using var writer = new StreamWriter(path);
            writer.WriteLine("Group Name\tItem Name\tAllocated Size");
            
            if (m_WidgetControllers != null)
            {
                foreach (var controller in m_WidgetControllers)
                {
                    foreach (var row in controller.Model.Rows)
                    {
                        writer.WriteLine($"{controller.Model.Title}\t{row.Name}\t{row.BaseSize.Committed}");
                    }
                }
        ...
        /// <summary>
        /// 自动化流程获取SummaryData数据
        /// </summary>
        public string GetData()
        {
            List<SummaryDataClass> result = new List<SummaryDataClass>(50);
            if (m_WidgetControllers != null)
            {
                foreach (var controller in m_WidgetControllers)
                {
                    SummaryDataClass tot = new SummaryDataClass();
                    tot.GroupName = controller.Model.Title;
                    tot.ItemName = "";
                    tot.AllocatedSize = controller.Model.TotalA;
                    result.Add(tot);
                    foreach (var row in controller.Model.Rows)
                    {
                        SummaryDataClass smc = new SummaryDataClass();
                        smc.GroupName = controller.Model.Title;
                        smc.ItemName = row.Name;
                        if (row.Name.Equals("Total Resident On Device"))
                            smc.AllocatedSize = row.BaseSize.Resident;
                        else
                            smc.AllocatedSize = row.BaseSize.Committed;
                        result.Add(smc);
                    }
                }
