...
namespace Unity.MemoryProfiler.Editor.UI
{
    public class MemoryProfilerViewController : ViewController
    {
        ...
        public void SetTabBarView(int index)
        {
            var viewcontroller = m_AnalysisViewController as AnalysisTabBarController;
            viewcontroller.SelectedIndex = index;
        }
        
        public string GetMemoryData(string typeName)
        {
            var viewcontroller = m_AnalysisViewController as AnalysisTabBarController;
            if (viewcontroller!=null)
            {
                foreach (var item in viewcontroller.ViewControllers)
                {
                    SummaryViewController smc = item as SummaryViewController;
                    UnityObjectsBreakdownViewController utc = item as UnityObjectsBreakdownViewController;
                    AllTrackedMemoryBreakdownViewController atm = item as AllTrackedMemoryBreakdownViewController;
                    if (smc!=null && smc.data.Equals(typeName))
                    {
                        return smc.GetData();
                    }
                    else if (utc!=null && utc.tableViewController.data.Equals(typeName))
                    {
                        return utc.tableViewController.GetData();
                    }
                    else if (atm!=null && atm.tableViewController.data.Equals(typeName))
                    {
                        return atm.tableViewController.GetData();
                    }
                }
            }
            else
        ...
        public bool IsViewLoadedComplte(string pageView)
        {
            var viewcontroller = m_AnalysisViewController as AnalysisTabBarController;
            if (viewcontroller != null)
            {
                foreach (var item in viewcontroller.ViewControllers)
                {
                    UnityObjectsBreakdownViewController unityView = item as UnityObjectsBreakdownViewController;
                    AllTrackedMemoryBreakdownViewController allView = item as AllTrackedMemoryBreakdownViewController;
                    if (unityView!=null && unityView.tableViewController.data.Equals(pageView) && !unityView.tableViewController.IsModelNone())
                    {
                        return unityView.IsViewLoaded;
                    }
                    else if (allView!=null)
                    {
                        if (allView.tableViewController!=null&&allView.tableViewController.data.Equals(pageView)&&!allView.tableViewController.IsModelNone())
                        {
                            return allView.IsViewLoaded;
                        }
                    }
                }
            }
